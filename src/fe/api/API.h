...
namespace fe
{
	namespace api
	{
		...
		inline void setLoggingLevel(LoggingLevel l)
		{
			spdlog::set_level(static_cast<spdlog::level::level_enum>(l));
		}

		extern util::FileCache fileCache;

		inline bool addFile(const std::string &file)
		{
		...
		inline void preprocess(const std::string &file)
		{
		...
		inline std::unique_ptr<core::lexer::TokenVector> lexer(const std::string &file)
		{
			auto f = fileCache.getFile(file);
		...
		inline bool runFile(const std::string &file)
		{
